...
{
    float3 views;
 	views.r = (p_over_du * _width * uv.x) + (p_over_dv * _height * uv.y) + fmod(colorSlant * _height * uv.y, p_over_du);
	views.g = (p_over_du * _width * uv.x) + (p_over_dv * _height * uv.y) + fmod(colorSlant * _height * uv.y, p_over_du);
	views.b = (p_over_du * _width * uv.x) + (p_over_dv * _height * uv.y) + fmod(colorSlant * _height * uv.y, p_over_du);
...
float3 calculateViewIndices(float2 normalized_display_coord) {
	...
	float user_offset = 0;
	//Unity flips the uv.y in openGL
#if SHADER_API_GLCORE || SHADER_API_GLES || SHADER_API_GLES3
	user_offset = 1.0 - p_over_dv * _height;
#endif
