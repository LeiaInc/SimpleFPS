...
namespace LeiaLoft
{
    ...
    public abstract class AbstractLeiaStateTemplate : ILeiaState, IReleasable
    {
        ...
        public virtual void DrawImage(LeiaCamera camera, LeiaStateDecorators decorators)
        {
            ...
            {
                double N = numViews;
                int min_view = (int)Math.Floor((N - 5) * 0.5);
                int max_view = (int)Math.Ceiling((N + 3) * 0.5);
                // Set shader min
                _material.SetInt("minView", min_view);
                _material.SetInt("maxView", max_view);
            }
            else
            {
                _material.SetInt("minView", 0);
                _material.SetInt("maxView", numViews - 1);
            }
            ...
            if (LeiaDisplay.Instance.shifter != null)
            {
                //peeling
                if (LeiaDisplay.Instance.viewPeeling)
                {
                    LeiaDisplay.Instance.shifter.peelControls.z = LeiaDisplay.Instance.getPeelOffsetForCameraShift().x;
                }
                else
                {
                    LeiaDisplay.Instance.shifter.peelControls.z = 0;
                    //LeiaDisplay.Instance.shifter.peelControls.y = LeiaDisplay.Instance.getPeelOffsetForCameraShift().y;
                    if (camera.BaselineScaling == 0)
                    {
                    ...
                    {
                        float zshift = 0;
                        if (LeiaDisplay.Instance.cameraDriven && LeiaDisplay.Instance.enableZCameraShift)
                        {
                            zshift = LeiaDisplay.Instance.getPeelOffsetForCameraShift().z + camera.ConvergenceDistance;
                        }

                        camera.CameraShift = new Vector3(
                            LeiaDisplay.Instance.getPeelOffsetForCameraShift().x / camera.BaselineScaling,
                            LeiaDisplay.Instance.getPeelOffsetForCameraShift().y / camera.BaselineScaling,
                            zshift
                            );
                    }
                }
